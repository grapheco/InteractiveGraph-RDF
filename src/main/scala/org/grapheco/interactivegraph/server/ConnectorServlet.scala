    ...
    if (command==null) {
      ...
      try {
        ...
        val ct: ContentTag = new ContentTag {
          def setContentType(ct: String): Unit = resp.setContentType(ct)

          def setContentLength(len: Int): Unit = resp.setContentLength(len)

          def setCatacterEncoding(en: String): Unit = resp.setCharacterEncoding(en)
        };
